...
test_recovery_reuses_current_release_commit_without_manifest_diff() {
  local origin="${TMP_DIR}/recovery-origin.git"
  local seed="${TMP_DIR}/recovery-seed"
  local work="${TMP_DIR}/recovery-work"
  create_origin_with_initial_commit "${origin}" "${seed}"
  ...
  local output="${TMP_DIR}/recovery-output.txt"
  local gh_output="${TMP_DIR}/recovery-github-output.txt"
  run_prepare "${work}" "1.2.3" "${dispatch_sha}" "${gh_output}" true >"${output}" 2>&1

  test "$(git -C "${work}" rev-parse HEAD)" = "${dispatch_sha}"
  test "$(git --git-dir="${origin}" rev-parse refs/heads/main)" = "${dispatch_sha}"
  grep -q "release_commit=${dispatch_sha}" "${gh_output}"
  grep -q "Reusing release commit ${dispatch_sha}" "${output}"
}
